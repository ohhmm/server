...
public:
  ...
  {
    static LEX_CSTRING name= {STRING_WITH_LEN("cast_as_uuid") };
    return name;
  }
...
public:
  ...
  String *val_str(String *to) override
  {
    return m_value.to_string(to, true) ? NULL : to;
  }
  my_decimal *val_decimal(my_decimal *to) override
  {
    my_decimal_set_zero(to);
  ...
  {
    StringBufferUUID tmp;
    m_value.to_string(&tmp, true);
    str->append(STRING_WITH_LEN("UUID'"));
    str->append(tmp);
    str->append('\'');
  }
  Item *get_copy(THD *thd) override
...
void Type_handler_uuid::make_sort_key_part(uchar *to, Item *item,
