...
/**
 * TODO Write this, mention it is stateful , and [start, stop)
 */
/**
  A filter implementation that allows GTID events to be included/excluded
  between certain start/stop points.
...
private:
  ...
  /* m_has_passed : Indicates whether or not the program is currently reading events from within this window. */
...
public:
  ...
  my_bool add_filter(Identifiable_gtid_event_filter *filter);
...
private:
  ...
  /*
    ...
    index. The index itself is found by a bitwise and, i.e.
        some_rpl_gtid.domain_id & m_domain_id_mask
   */
  gtid_filter_element **m_filters_by_domain_id;

  gtid_filter_element *try_find_filter_element_for_domain(uint32);
  gtid_filter_element *find_or_create_filter_element_for_domain(uint32);
  Window_gtid_event_filter *find_or_create_window_filter_for_domain(uint32);
};
