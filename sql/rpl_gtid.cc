...
Delegating_gtid_event_filter::Delegating_gtid_event_filter()
{
  ...
  m_domain_id_mask= 0xf;

  m_filters_by_domain_id= (gtid_filter_element **) my_malloc(
    PSI_NOT_INSTRUMENTED,
    (m_domain_id_mask + 1) * sizeof(gtid_filter_element *),
    MYF(MY_WME)
  );

  DBUG_ASSERT(m_filters_by_domain_id != NULL);

  for (i = 0; i <= m_domain_id_mask; i++)
  {
    m_filters_by_domain_id[i]= NULL;
  }
...
/**
  Deconstructor deletes:
   ...
   2) All gtid_filter_nodes allocations
   3) All gtid_filter_element allocations
 */
Delegating_gtid_event_filter::~Delegating_gtid_event_filter()
{
  uint32 i;
  for (i = 0; i <= m_domain_id_mask; i++)
  {
    gtid_filter_element *filter_element= m_filters_by_domain_id[i],
  ...
  my_free(m_filters_by_domain_id);
...
gtid_filter_element *
Delegating_gtid_event_filter::try_find_filter_element_for_domain(
    uint32 domain_id)
{
  // Add this into the domain id list
  uint32 map_idx= domain_id & m_domain_id_mask;

  gtid_filter_element *filter_idx= m_filters_by_domain_id[map_idx];

  /* Find list index to add this filter */
  while (filter_idx)
  {
    if (filter_idx->identifier == domain_id)
    {
      // Filters on this domain_id already exist
      // TODO
      break;
    }
...
gtid_filter_element *
Delegating_gtid_event_filter::find_or_create_filter_element_for_domain(
    uint32 domain_id)
{
  // Add this into the domain id list
  uint32 map_idx= domain_id & m_domain_id_mask;
  ...
  if (filter_idx == NULL)
  {
    ...
    filter_idx->identifier= domain_id;
    ...
    if (prev_idx == NULL)
    {
      ...
      m_filters_by_domain_id[map_idx]= filter_idx;
    }
    else
    {
...
my_bool Delegating_gtid_event_filter::add_filter(
    Identifiable_gtid_event_filter *filter)
{
  uint32 filter_flags= filter->get_lookup_flags();
  my_bool did_add= FALSE;

  /*
    Depending on how this filter is identified, add it to the corresponding
    list
    */
  if (filter_flags & Identifiable_gtid_event_filter::BY_DOMAIN_ID)
  {
    // Add this into the domain id list
    gtid_filter_element *filter_element=
        find_or_create_filter_element_for_domain(filter->get_filter_identifier());

    if (filter_element->filter)
    {
      // This domain already has a filter, we can't add another one
      goto end;
    }

    filter_element->filter= filter;
    did_add= TRUE;
  }
...
my_bool Delegating_gtid_event_filter::exclude(rpl_gtid *gtid)
{
  ...
  gtid_filter_element *filter_element= try_find_filter_element_for_domain(gtid->domain_id);
  if (filter_element)
  {
...
Delegating_gtid_event_filter::find_or_create_window_filter_for_domain(
    uint32 domain_id)
{
  gtid_filter_element *filter_element=
      find_or_create_filter_element_for_domain(domain_id);
...
my_bool Delegating_gtid_event_filter::add_start_gtid(rpl_gtid *gtid)
{
  Window_gtid_event_filter *filter_to_update=
      find_or_create_window_filter_for_domain(gtid->domain_id);
  return filter_to_update->set_start_gtid(gtid);
}

my_bool Delegating_gtid_event_filter::add_stop_gtid(rpl_gtid *gtid)
{
  Window_gtid_event_filter *filter_to_update=
      find_or_create_window_filter_for_domain(gtid->domain_id);
